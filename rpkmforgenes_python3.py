	...
	try:
		...
		for line in fileh:
			try:
				...
				if maxNM >= 0 and any((field.startswith('NM:i:') or field.startswith('nM:i:')) and int(field[5:]) > maxNM for field in p[11:]): continue # too many mismatches/indels
				if readlength_dict is not None:
...
def readsfrombam(filename, justreadnum, readlength_dict):
	...
	try:
		...
		for read in fileh:
			...
			if maxNM >= 0 and any(((key=='NM' or key == 'nM') and val > maxNM) for key, val in read.tags): continue # too many mismatches/indels
			try:
...
def readsfrombam_se(filename, justreadnum, readlength_dict):
	...
	try: 
		...
		if str(e) == "file does not contain alignment data": return
		raise
...
def readsfrombam_with_barcodes_as_iterable(filename, readlength_dict):
	try: 
		fileh = pysamopen(filename)
	except IOError:
		if vocal: print("Warning: No such file", filename)
		...
		for read in fileh:
			...
			if USESTRANDINFO:
				if mapends and read.flag & 0x80:
					...
					chromosome += strand
			start = read.pos
			yield read.get_tag('BC'), addread(chromosome, start, start+length, strand, mapends and (read.flag & 2))
	finally:
		fileh.close()

def readsfrombam_bc_gen(infiles, names, readlength_dict):
	if removemultimappers and vocal: print("Warning: The -unique option was ignored, unsupported for this format")

	for filename, name_l in zip(infiles, names):
		previous_barcodes = dict()
		exit_now = False
		read_input = iter(readsfrombam_with_barcodes_as_iterable(filename, readlength_dict))
		last_read = next(read_input)
		while not exit_now:
			def localgen():
				nonlocal last_read, exit_now
				yield last_read[1]
				for new_read in read_input:
					if new_read[0] != last_read[0]:
						last_read = new_read
						return
					yield new_read[1]
					last_read = new_read
				exit_now = True
			barcode_this_time = last_read[0]
			if barcode_this_time in previous_barcodes:
				if vocal:
					print("Warning: Found barcode "+barcode_this_time + " again, bam file is not sorted by BC tag?")
				previous_barcodes[barcode_this_time] += 1
				barcode_this_time += 'occurance%i'%previous_barcodes[barcode_this_time]
			else:
				previous_barcodes[barcode_this_time] = 1
			yield (name_l + '_' + barcode_this_time if name_l else barcode_this_time), localgen()
...
def merge(o_infiles, o_outfile):
	...
	with openfile(o_outfile, 'w') as outfh:
		...
		for linenum in range(numgenes):
			print(genelines[linenum] + readlines[linenum], file=outfh)
...
def getarguments(flag):
	...
	ret = []
	index = flagindex + 1
	while index < len(sys.argv) and (sys.argv[index] == '' or sys.argv[index][0] != "-"):
...
def main():
	...
	global USESTRANDINFO, WINDOWSIZE
	...
	if len(sys.argv) < 2 or testargumentflag("--help") or testargumentflag("-h"):
		...
		print(" -keephap to not remove haplotype chromosome (_hap/_alt) annotation")
		...
		print("-genePred_nobinID if annotation file uses the output of gtfToGenePred -genePredExt, which does not have the extra numerical field at the beginning but starts with transcript ID instead")
		print(" -bedann tab-separated 0-based bed file, chromosome start end and 9 optional fields")
		print(" -ensgtfann for gtf format gene annotation (slow, generally better to convert to bed or genepred)")
		print(" -addchr to change a read aligning to 1 to chr1, 2 to chr2 etc")
		print("Input formats (flags generally not needed, will detect from file ending):")
...
#		print " -bam BAM format, and to remove non-unique hits" # broken read counting
		...
		print(" -bambc BAM or SAM format, sorted by BC tag which separates samples")
		...
		print(" -tmm to run TMM normalisation on top of RPKM (or TPM if -tpm is used) normalisation")
		...
		print(" -maxobs followed by a number to limit the number of input samples or barcodes")
		...
		print(" Consider using the -csv flag for a more standard output format")
		return 0
	...
	elif testargumentflag("-genePred_nobinID"): annotationtype = 8
	else:
	...
	if testargumentflag("-blocksize"): WINDOWSIZE = int(getargument("-blocksize"))
	else: WINDOWSIZE = 5000
	...
	elif testargumentflag("-bambc"): infileformat = "bambc"
	...
	if testargumentflag("-maxobs"):
		maxobs = int(getargument("-maxobs"))
	else: maxobs = None
	...
	if not testargumentflag("-keephap"):
		...
		ignoredchrfragments.append("_alt")
		keephap = False
	else:
		keephap = True
	...
	if annotationtype == 7: # -ensgtfann
		annlines = [Cline(L, min(e[-1] for e in L)) for L in list(lines_per_ID.values())]
	annlines.sort(key=lambda cline: (cline.chromosome, cline.start))
	...
	for gene in genes:
		for exon in gene.exons:
			...
			if lw_chr <= w_end:
				allwindows[chrID].extend([] for i in range(w_end-lw_chr+1))
	...
	if infileformat == "bambc":
		from collections import defaultdict
		readlength_dict = defaultdict(int) if global_readlength is None else None
		infile_bambc_gen = readsfrombam_bc_gen(infiles, names, readlength_dict)
		names = []
	for infile_i, (infile_name, infile) in enumerate(infile_bambc_gen if infileformat == "bambc" else zip(names, infiles)):
		if infile_i >= maxobs: break
		from collections import defaultdict
		readlength_dict = defaultdict(int) if global_readlength is None else None
		for gene in genes:
			for exon in gene.exons:
		...
		if infileformat == "bambc":
			readgen = lambda filename, justreadnum, readlength_dict: filename
			names.append(infile_name)
		else:
		...
		if randomreads or skipiffewreads:
			if infileformat == "bambc":
				if vocal: print("Error: randomreads and skipiffewreads not implemented with bambc")
				return 1
		...
		for gene in genes:
			...
			Dindices = list(range(len(gene.transcripts)))	# which element in gene.transcripts that an element in D corresponds to
			redofornegative = 1
			counter = 0
			...
			while redofornegative:			
				...
				# 
				try: detsign, logdet = linalg.slogdet(LTL)
				except AttributeError:
					determinant = numpy.linalg.det(LTL)
					if determinant == 0:
						detsign = 0
						logdet = -100
					else:
						detsign = 1 if determinant > 0 else -1
						logdet = numpy.log(abs(determinant))
		...
		if vocal: print("Has calculated expression values for", infile_name)
	...
	if outputformat == 'v2':
		print("\t".join(["#samples"]+names), file=outfileh)
		print("\t".join(["#allmappedreads"] + [str(v) for v in totalreadspersample]), file=outfileh)
		if forcedtotal:
			print("\t".join(["#normalizationreads"] + [str(v) for v in forcedtotal]), file=outfileh)
		else:
			print("\t".join(["#normalizationreads"] + [str(v) for v in normreadfactors]), file=outfileh)
		print("#arguments\t"+" ".join(sys.argv) + "\ttime:" + time.asctime() + '\tversion:'+lastmodified, file=outfileh)
		...
		for tu in tunits:
			...
			if addreadcount:
				if mapends_ceil:
					print('\t'.join([tu.name1, tu.name2] + [str(v) for v in tu.rpkms] + [str(int(math.ceil(v))) for v in tu.reads]), file=outfileh)
				else:
					print('\t'.join([tu.name1, tu.name2] + [str(v) for v in tu.rpkms] + [str(v)[:-2] if str(v).endswith('.0') else str(v) for v in tu.reads]), file=outfileh)
			else:
				print('\t'.join([tu.name1, tu.name2] + [str(v) for v in tu.rpkms]), file=outfileh)
	elif outputformat == 'table':
		print("\t".join(["Gene"]+names), file=outfileh)
		if originalorder:
			tunits.sort(key=lambda unit: unit.sortnum)
		for tu in tunits:
			...
			if addreadcount:
				if mapends_ceil:
					print('\t'.join([name] + [str(int(math.ceil(v))) for v in tu.reads]), file=outfileh)
				else:
					print('\t'.join([name] + [str(v)[:-2] if str(v).endswith('.0') else str(v) for v in tu.reads]), file=outfileh)
			else:
				print('\t'.join([name] + [str(v) for v in tu.rpkms]), file=outfileh)
	outfileh.close()
	...
	if exportannotation:
		...
		print("#numbers for " + infiles[-1], file=outfileh)
		print('#genename\tchromosome\texons', file=outfileh)
		print('#exon:\tstart\tend\tlength\treads\ttranscript_IDs', file=outfileh)
		for gene in genes:
			exons = [exon for exon in gene.exons if not exon.forbidden]
			print(gene.name + '\t' + gene.chromosome +'\t' + str(len(exons)), file=outfileh)
			for exon in exons:
				print('\t'.join(map(str,['exon:', exon.start, exon.end, exon.length, exon.reads] + [tx.ID+'|'+tx.genename for tx in exon.transcripts])), file=outfileh)
...
def calcuniqlength(genes, readlength_dict, filesuffix):
	readlength_dict_weights = dict((r,float(n)/sum(readlength_dict.values())) for r,n in list(readlength_dict.items()))
	for gene in genes:
		# forbidden exons do not belong to transcripts
